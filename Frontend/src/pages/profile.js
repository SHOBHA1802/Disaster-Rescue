...
function Profile() {
  ...
  const [loadingChat, setLoadingChat] = useState(0)
  ...
  const fetchOptions = async () => {
    ...
    //   setPending(JSON.parse(pending));
    //   setHashNotif(JSON.parse(notif));
    //   setHashFriends(JSON.parse(friends));
    //   setLoading(false);
    ...
    try {
      ...
      arr.forEach((curr_val) => (pending2[curr_val.Email2] = 1));
      setPending(pending2);
      localStorage.setItem("pending", JSON.stringify({ pending: pending2 }));
      ...
      setHashNotif(f);
      localStorage.setItem("notif", JSON.stringify({ notif: f }));
      ...
      <>
        ...
        <div className="profile-container">
          ...
          <section>
            ...
            {console.log("This is chat", loadingChat)}
            {loadingChat ? <Loading /> : ""}
            {chat == 0 ?
              <>
