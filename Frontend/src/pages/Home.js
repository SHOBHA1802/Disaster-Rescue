    ...
    if (!email) {
      ...
      try {
        ...
        if (d_now.data.address) {
          ...
          setLat(d_now.data.lat);
          setLon(d_now.data.lon);
          lat_now = d_now.data.lat;
          lon_now = d_now.data.lon;
          lat_now = Number(lat_now);
          lon_now = Number(lon_now);
          localStorage.setItem("latitudes", JSON.stringify({ lat: lat_now, lon: lon_now, }));
      ...
      return (
        <div>
          ...
          <div className="farm-container">
            <h1>Disaster Rescue</h1>
            <p className="agro-para">
              Rescue efficiently for a disaster with the help of our AI tools
            </p>
          </div>
